...
- **All hand-written `SCR_EditableEntityComponent { m_EditableEntityFlags "PLACEABLE" m_sDisplayName "..." }` blocks in our prefabs are wrong and silently do nothing** -- confirmed via `WORLD (E): Unknown keyword/data 'm_EditableEntityFlags'`/`'m_sDisplayName'` on placement. This affects every prefab we've hand-written this way (all 8+ `.et` files under `Prefabs/`), not just new ones -- earlier ones just happened not to surface this specific error message when we checked them. **The correct, BI-documented way to make a prefab properly PLACEABLE is Workbench's own "Create/Update Selected Editable Prefabs" plugin** (Plugins menu, or Ctrl+Shift+U), which generates a correct editable variant from a source prefab automatically -- "All editable entities which use component prefab `Default_SCR_EditableEntityComponent.ct` are already flagged as PLACEABLE" (BI wiki, Game Master: Editable Entities Configuration). We should stop hand-writing the `SCR_EditableEntityComponent` block ourselves and instead: (1) create prefabs with only our own MCF component, (2) select them in Resource Browser, (3) run that plugin to generate the real editable/placeable variant. **Not yet done for any of our prefabs** -- this needs to happen before Game Master can actually place any of them, despite the Entity Catalog (`Configs/EntityCatalogs/MCF_EntityCatalog.conf`) already listing them.
