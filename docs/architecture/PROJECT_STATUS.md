...
- **`Prefabs/Systems/Milsim.et`** -- our own GameMode prefab, inheriting from the confirmed vanilla base `{1B76F75A3175E85C}Prefabs/MP/Modes/Plain/GameMode_Plain.et`, with `MCF_Core_GameModeComponent`/`MCF_Core_TickManagerComponent`/`MCF_Core_GameLoopComponent` added. Building it this way (a `prefab_create` with the correct `parentPrefab` GUID) was necessary -- using `prefabType: "gamemode"` with no parent produces a `GenericEntity`-rooted prefab that fails at runtime ("required type=SCR_BaseGameMode! This is not allowed!"). Placing it standalone in an incomplete test world (missing `SCR_FactionManager`/`SCR_LoadoutManager`/`SCR_SpawnPoint`, all confirmed mandatory per BI's own General Game Mode Setup docs) **crashed Workbench** -- `SCR_RespawnSystemComponent is missing SCR_SpawnLogic!` followed by an access violation. Our three MCF components themselves resolved and attached with zero errors before the crash; the crash traces to the incomplete test world, not our code. **Not tested in a complete scenario with the other mandatory pieces present** -- do that before relying on this prefab for anything real.
