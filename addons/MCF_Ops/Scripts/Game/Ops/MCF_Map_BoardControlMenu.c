...
//! own root. Do the same and the markers, the right-click menu, the ruler, the
//! tools and MCF's own freehand drawing are the game's own -- not
//! reimplemented, not approximated, and they will keep working when the game
//! changes them.
//!
//! WHY THERE IS NO DRAWING CODE HERE. Because this window opens the gadget map
//! config, and that config carries SCR_MapDrawingUI, which MCF mods (see
//! MCF_Map_DrawingUI.c). The freehand drawing is therefore already in this
//! window for the same reason it is already in the player's M map: it belongs
//! to the map, not to whoever opened it.
//!
...
class MCF_Map_BoardControlMenu : ChimeraMenuBase
{
	...
	//! enough for the board to look live once it chases between them; every
	//! push is an RPC, so this is the one number that costs a server.
	...
	override void OnMenuOpen()
	{
		...
		// right-click menu, the same markers, the same drawing.
	...
	override void OnMenuUpdate(float tDelta)
	{
		...
		// WHAT THE ROOM SEES. The driver's view, ten times a second, onto the
		// board -- so the panel in the world pans and zooms with them while
		// they work.
		float centreX, centreZ;
